...
# built in grey colors (0 = black 100 = pure white)
...
# scale_color_manual() (line,points)
...
# consinuous classification for color gradient
...
# custom sequential gradient (2 color)
p_grad + scale_color_gradient(low="green",
                                   high='red')

# custom divergint gradient (3 colors)
mid <- median(d$hwy)
...
p4 + scale_fill_gradient2(midpoint=19,
                          ...
                          mid=gray(0.8),
                          high='darkblue')
# use viridis condinuous scale
p4 + scale_fill_viridis_c()

# option viridis, cividis, magma, inferno, plasma
