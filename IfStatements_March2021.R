...
# most useful in if statement is %in% or is.element
...
# else statement must appear on the same line as the previous expression
...
# it will operate on the very first element in that vector
...
# ifelse(test,yes,no)
# 'test' is an object which can be coerced into a logical 
# TRUE/FALSE
...
# females lays, on average, 10.2 eggs, following a poisson distribution. lambda = 10.2. However there is a 35% chance of parasitism, in which case, no eggs are laid. Here is a random sample of eggs laid for a group of 1000 individuals\

tester <- runif(1000) # start with random uniform element 
